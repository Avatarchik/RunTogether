    ...
    public class PersonalProfileState : State<PersonalProfile>
    {
                ...
                new ListView(
                    children: new List<Widget>
                    {
                        new Container(
                            ...
                            child:
                            new ListTile(leading:_buildAvatar(),title:_buildNickName(),subtitle:_buildMotto(),trailing:_buildTrailing())
                        ),
                        ...
                        new Container(
                            ...
                            child:new Column(
                                children:new List<Widget>
                                {
                                    new ListTile(title:new Text("Leaderboard"),leading:new Icon(Icons.leaderboard),onTap:
                                        () =>
                                        {
                                            Route tmpRoute = new PageRouteBuilder(
                                                ...
                                                transitionsBuilder:((buildContext, animation, secondaryAnimation, child) => 
                                                    new FadeAndMovePageTransition(routeAnimation:animation,child:child,beginDirection:new Offset(2f,0f),endDirection:Offset.zero))
                                            ); 
                                            Navigator.push(context: context, route:tmpRoute);
                                        }),
                                    new Divider(indent:70),
                                    new ListTile(title:new Text("Setting"),leading:new Icon(Icons.setting),onTap: () =>
                                    {
                                        Route tmpRoute = new PageRouteBuilder(
        ...
        private void OnTapProfileListTile()
        {
            Route tmpEditProfileRoute = new PageRouteBuilder(
                ...
                transitionsBuilder:((buildContext, animation, secondaryAnimation, child) => 
                new FadeAndMovePageTransition(routeAnimation:animation,child:child,beginDirection:new Offset(1f,0f),endDirection:Offset.zero))
            ); 
